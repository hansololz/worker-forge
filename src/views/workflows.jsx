/* ============================================================
   Workflows list + detail + pipeline + editor + prepare-run.
   Data via the model + API.
   ============================================================ */
import React from 'react'
import { e, Icon, Btn, Select } from '../ui'
import { DB } from '../model'
import { useEditorDraft } from '../editorDraft'
import { api } from '../api'
import { RunsUI } from './runs'
import { TriggersPanel } from './triggers'

const { useState, useEffect, useRef } = React

// Re-render once a second while `active`, so the scheduled countdown stays live
// to the second. Returns the current epoch ms.
...
// "in 2h 5m 3s" — time from now until the ISO instant, to the second. Once a
// higher unit appears every lower one follows (so "in 3d 0h 0m 5s"). Past/now → "now".
function fmtCountdown(iso, nowMs) {
  if (!iso) return '—'
  let s = Math.round((new Date(iso).getTime() - nowMs) / 1000)
  if (s <= 0) return 'now'
...
function stagesOf(w) { return (w.stages || []).map(el => Array.isArray(el) ? el.slice() : [el]) }
...
function resolveParam(p, taskId, wfParams, params) {
  ...
  const inherited = hasWf ? wfRaw : taskDefault
...
/* ---------------- WORKFLOWS LIST ---------------- */
export function WorkflowsList({ ctx }) {
    ...
    e('div', null,
      (() => {
        ...
        return e('div', { style: { display: 'flex', flexDirection: 'column', gap: 2 } },
          ...
          everRan && w.lastRunAt && e('span', { className: 'mono', style: { fontSize: 11, color: 'var(--tx-lo)' } }, w.lastRunAt))
      })()),
    e('div', null,
      sched.type === 'cron'
        ? e('div', { style: { display: 'flex', flexDirection: 'column', gap: 2 } },
            e('span', { className: 'mono', style: { fontSize: 13, color: 'var(--tx)' } }, fmtCountdown(sched.nextAt, now)),
...
/* ---------------- WORKFLOW DETAIL ---------------- */
export function WorkflowDetail({ ctx }) {
  ...
  if (!w) return e('div', { className: 'page page-wide fadein' }, e('div', { className: 'empty', style: { padding: '60px 0' } }, 'Workflow not found.'))
...
function RecentRuns({ w, ctx }) {
  const { RunRow, Pager, RUN_COLS, PAGE_SIZE } = RunsUI
  const [all, setAll] = useState(null)
  const [total, setTotal] = useState(0)
  const [page, setPage] = useState(1)
  useEffect(() => {
    let live = true
    api.listExecutions({ workflow_id: w.id, page }).then(res => {
      if (!live) return
      setAll(res.items || []); setTotal(res.total || 0)
    }).catch(() => { if (live) { setAll([]); setTotal(0) } })
    return () => { live = false }
  }, [w.id, page])
  const pageCount = Math.max(1, Math.ceil(total / PAGE_SIZE))
  const cur = Math.min(page, pageCount)
  const start = (cur - 1) * PAGE_SIZE
  const end = Math.min(start + (all ? all.length : 0), total)
    ...
    total
      ? e(React.Fragment, null,
          e('div', { className: 'wf-head', style: { gridTemplateColumns: RUN_COLS } },
            ...
            e('span', null, '')),
          e('div', null,
            (all || []).map(r => e(RunRow, { key: r.id, r, onClick: () => ctx.nav({ view: 'run', runId: r.id, workflowId: w.id }) }))),
          e(Pager, { page: cur, pageCount, onPage: setPage, total, start, end }))
      : e('div', { className: 'empty', style: { padding: 30 } }, all == null ? 'Loading…' : 'No runs yet'))
}
...
export function WorkflowEdit({ ctx }) {
  ...
  const freshDraft = () => creating
    ...
    : copyWfVer(cur || {})
  // Load a chosen historical version into the draft (null when the workflow is gone).
  const loadWfVersion = (ver) => {
    const w = ctx.workflows.find(x => x.id === ctx.state.workflowId)
    if (!w) return null
    const vs = buildWfVersions(w)
    const v = vs.find(x => x.version === ver) || vs[0]
    return { ...copyWfVer(v ? v.data : w), id: w.id }
  }
  // Draft state (entity/version switch guards; see useEditorDraft). Loads fresh —
  // nothing is preserved across navigation.
  const { draft, setDraft, selVer, setSelVer } = useEditorDraft({
    key: cacheKey, curVer, fresh: freshDraft, loadVersion: loadWfVersion,
    onSwitchEntity: () => setTab(seedTab(ctx.state)),
  })
  ...
  function save() {
    ...
    ctx.saveWorkflow({ id: draft.id, name: draft.name, desc: draft.desc, wfParams: draft.wfParams || {}, params: draft.params, exec: draft.exec, triggers: draft.triggers || [], stages: draft.stages.filter(s => s.length > 0).map(s => s.slice()) })
  }
  ...
  return e('div', { className: 'page fadein' },
    ...
    e('div', { style: { paddingTop: 18 } },
      ...
      tab === 'triggers' && e('div', { className: 'fadein' },
        e(TriggersPanel, { triggers: draft.triggers || [], setTriggers, ctx }))))
}

/* ---------------- RUN PREPARE (parameter review before launch) ---------------- */
// Each task occurrence in the pipeline is its own "slot", keyed by its flattened
// position over the run's ENABLED tasks (matches the run view + backend numbering).
// The SAME task used in two stages therefore gets independent inputs and values.
const pKey = (slot, k) => slot + '::' + k

export function RunPrepare({ ctx }) {
  const w = ctx.workflows.find(x => x.id === ctx.state.workflowId)
  const prefill = ctx.state.prefill

  // Walk the pipeline stage by stage. Disabled tasks don't run, so they're
  // skipped and don't advance the slot index; every enabled task advances it
  // (so it lines up with the run view) and becomes a configurable slot —
  // including tasks with no declared parameters, so they can still have ad-hoc
  // parameters added for this run. Slots are grouped under their stage.
  const stageBlocks = []   // [{ si, slots: [{ idx, s }] }]
  if (w) {
    let idx = 0
    stagesOf(w).forEach((step, si) => {
      const slots = []
      step.forEach(id => {
        if (w.exec && w.exec[id] && w.exec[id].enabled === false) return
        const cur = idx++
        const s = DB.taskById[id]
        if (s) slots.push({ idx: cur, s })
      })
      if (slots.length) stageBlocks.push({ si, slots })
    })
  }
  const allSlots = stageBlocks.flatMap(b => b.slots)
  ...
  // Ad-hoc parameters the user adds for THIS run, per slot: { slotIdx: [{k,v}] }.
  // Restored from a re-run's params (any key the task doesn't declare).
  ...
  // A slot's effective value: what's typed, else the workflow's per-task
  // override, else the task's env default.
  const baseOf = (s, p) => {
    const ov = w.params && w.params[s.id] && w.params[s.id][p.k]
    return (ov != null && ov !== '') ? ov : (p.v != null ? p.v : '')
  }
  const effOf = (idx, s, p) => { const v = vals[pKey(idx, p.k)]; return (v !== undefined && v !== '') ? v : baseOf(s, p) }
  // Which task version this run will use, and whether that's the latest. The
  // workflow's exec pin ("latest" or a fixed number) decides it.
  const verOf = (s) => {
    const ex = (w.exec && w.exec[s.id]) || {}
    const pin = ex.version !== undefined ? ex.version : 'latest'
    const curV = s.version || 1
    const runVer = pin === 'latest' ? curV : pin
    return { runVer, curV, latest: pin === 'latest' || pin >= curV }
  }
  const setVal = (idx, k, v) => setVals(st => ({ ...st, [pKey(idx, k)]: v }))
  const addExtra = (idx) => setExtras(ex => ({ ...ex, [idx]: [...(ex[idx] || []), { k: '', v: '' }] }))
  const setExtra = (idx, i, field, val) => setExtras(ex => ({ ...ex, [idx]: (ex[idx] || []).map((r, j) => j === i ? { ...r, [field]: val } : r) }))
  const removeExtra = (idx, i) => setExtras(ex => ({ ...ex, [idx]: (ex[idx] || []).filter((_, j) => j !== i) }))

  // every required (slot, param) pair that is still empty
  const missing = []
  allSlots.forEach(({ idx, s }) => (s.env || []).forEach(p => { if (p.required && !effOf(idx, s, p)) missing.push(p.k) }))
  const canRun = missing.length === 0
  const missKeys = Array.from(new Set(missing))

  function launch() {
    if (!canRun) return
    const params = {}                       // { slotIdx: { KEY: value } }
    allSlots.forEach(({ idx, s }) => (s.env || []).forEach(p => {
      const v = effOf(idx, s, p)
      if (v !== '') { (params[String(idx)] = params[String(idx)] || {})[p.k] = v }
    }))
    // fold in any ad-hoc parameters added for this run (named + non-empty)
    allSlots.forEach(({ idx }) => (extras[idx] || []).forEach(r => {
      const k = (r.k || '').trim()
      if (k && r.v !== '') { (params[String(idx)] = params[String(idx)] || {})[k] = r.v }
    }))
  ...
  return e('div', { className: 'page page-wide fadein' },
    e('div', { className: 'ph' },
      e('div', { style: { minWidth: 0 } },
        ...
        e('p', null, prefill && prefill.fromRun
          ...
          : 'Review the parameters for this run, grouped by stage and task. Each task is configured independently within its stage — required fields must be set before the flow can start.')),
      e('div', { className: 'ph-actions' },
    ...
    allSlots.length === 0
      ? e('div', { className: 'card' }, e('div', { className: 'empty', style: { padding: 30 } }, 'This workflow has no tasks — nothing to configure before running.'))
      : e('div', { style: { display: 'flex', flexDirection: 'column', gap: 26 } },
          stageBlocks.map(b => e('div', { key: b.si, style: { display: 'flex', flexDirection: 'column', gap: 12 } },
            e('div', { style: { display: 'flex', alignItems: 'center', gap: 10 } },
              e('span', { className: 'stage-tag-n', style: { fontSize: 11, padding: '2px 10px', lineHeight: 1.7 } }, 'Stage ' + (b.si + 1)),
              e('span', { style: { fontSize: 11, fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.07em', color: 'var(--tx-dim)' } },
                b.slots.length + (b.slots.length === 1 ? ' task' : ' tasks') + (b.slots.length > 1 ? ' · all at once' : ''))),
            b.slots.map(({ idx, s }) => e(PrepTaskCard, { key: idx, idx, s, vals, setVal, effOf, baseOf, ver: verOf(s), extras: extras[idx] || [], addExtra, setExtra, removeExtra }))))))
}

function PrepTaskCard({ idx, s, vals, setVal, effOf, baseOf, ver, extras, addExtra, setExtra, removeExtra }) {
  const env = s.env || []
  const missingN = env.filter(p => p.required && !effOf(idx, s, p)).length
  const addedN = extras.length
  const noParams = env.length === 0 && extras.length === 0
  return e('div', { className: 'card' },
    e('div', { className: 'card-h prep-card' },
      e(Icon, { name: s.icon, size: 15, style: { color: 'var(--tx-lo)', flex: 'none' } }),
      e('h3', null, s.name),
      ver && e('span', { className: 'ver-tag mono', style: { fontSize: 11, padding: '1px 7px', flex: 'none' } }, 'v' + ver.runVer),
      ver && (ver.latest
        ? e('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 10, fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--st-ok)', flex: 'none' } },
            e('span', { className: 'dot st-ok' }), 'latest')
        : e('span', { title: 'v' + ver.curV + ' is available', style: { display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 10, fontFamily: 'var(--mono)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--st-warn)', flex: 'none' } },
            e(Icon, { name: 'alert', size: 11 }), 'v' + ver.curV + ' available')),
      e('span', { className: 'sub', style: { marginLeft: 'auto' } },
        (env.length === 0 ? 'no parameters' : env.length + (env.length === 1 ? ' parameter' : ' parameters')) + (addedN ? ' · ' + addedN + ' added' : '') + (missingN ? ' · ' + missingN + ' missing' : ''))),
    e('div', { className: 'card-b', style: { padding: 0 } },
      noParams && e('div', { className: 'empty', style: { padding: '13px 16px', textAlign: 'left', fontSize: 12.5, color: 'var(--tx-lo)' } },
        'This task takes no parameters. Add one below to pass an ad-hoc parameter for this run.'),
      env.map(p => e(PrepParamRow, { key: p.k, idx, s, p, vals, setVal, effOf, baseOf })),
      extras.map((r, i) => e(PrepExtraRow, { key: 'x' + i, onChange: (f, v) => setExtra(idx, i, f, v), onRemove: () => removeExtra(idx, i), p: r })),
      e('div', { style: { borderTop: '1px solid var(--line-soft)', margin: (noParams ? '0' : '5px') + ' 0 0' } }),
      e(Btn, { variant: 'ghost', size: 'sm', icon: 'plus', style: { margin: '10px 16px 11px', borderStyle: 'dashed' }, onClick: () => addExtra(idx) }, 'Add parameter')))
}

function PrepExtraRow({ p, onChange, onRemove }) {
  return e('div', { className: 'prep-row' },
    e('div', { className: 'prep-k' },
      e('input', { className: 'input mono', style: { flex: 1, minWidth: 0 }, placeholder: 'PARAM_NAME', value: p.k, onChange: ev => onChange('k', ev.target.value) }),
      e('span', { className: 'param-req', style: { color: 'var(--accent)', background: 'var(--accent-dim)', borderColor: 'transparent' } }, 'added')),
    e('div', { className: 'prep-v' },
      e('input', { className: 'input mono', value: p.v, placeholder: 'value', onChange: ev => onChange('v', ev.target.value) }),
      e(Btn, { variant: 'ghost', size: 'sm', icon: 'x', style: { color: 'var(--st-fail)', flex: 'none' }, onClick: onRemove, title: 'Remove parameter' })))
}

function PrepParamRow({ idx, s, p, vals, setVal, effOf, baseOf }) {
  const def = baseOf(s, p)
  const isMissing = p.required && !effOf(idx, s, p)
  const vKey = pKey(idx, p.k)
  return e('div', { className: 'prep-row' + (isMissing ? ' missing' : '') },
    ...
    e('div', { className: 'prep-v' },
      e('input', { className: 'input mono' + (isMissing ? ' miss' : ''),
        value: vals[vKey] !== undefined ? vals[vKey] : def,
        placeholder: def ? '' : (p.required ? 'value required' : 'value (optional)'),
        onChange: ev => setVal(idx, p.k, ev.target.value) }),
