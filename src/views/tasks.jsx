...
import { e, Icon, Btn, Select, highlightCode } from '../ui'
...
function envRowError(row, i, env) {
  const k = (row.k || '').trim()
  if (k === '') return ((row.v || '').trim() !== '' || row.required) ? 'Key required' : null
...
/* ---------------- TASKS LIBRARY ---------------- */
export function TasksLibrary({ ctx }) {
...
function TaskCard({ s, ctx }) {
  return e('div', { className: 'task-card', onClick: () => ctx.openTask(s.id) },
    e('div', { className: 'sc-top' },
      ...
      e('div', { style: { minWidth: 0 } }, e('h4', null, s.name)),
...
export function TaskDetail({ ctx }) {
...
/* ---------------- TASK EDITOR ---------------- */
export function TaskEditor({ ctx }) {
  ...
  const cacheKey = taskId || '__new'
  const [openStep, setOpenStep] = useState(0)
  // Load a chosen historical version (or the saved base) into the draft.
  const loadTaskVersion = (ver) => {
    const v = isNew ? null : (versions.find(x => x.version === ver) || versions[0])
    return copyVer(v ? v.data : base)
  }
  ...
  const { draft, setDraft, selVer, setSelVer } = useEditorDraft({
    key: cacheKey, curVer, fresh: () => copyVer(base), loadVersion: loadTaskVersion,
    onSwitchEntity: () => { setTab('settings'); setOpenStep(0) },
    onSelectVersion: () => setOpenStep(0),
  })
  ...
  const usedByWfs = isNew ? [] : ctx.workflows.filter(w => w.stages.flat().includes(taskId))
