...
| `bg.band`        | `#F7F5F2` | The "chrome band" — title bar + header row + table-header strip, painted as one continuous color so they read as a single unit (see "The header band" below). One shade off `bg.base`. |
...
Matching the title bar to the body is the floor; the next step up in taste is to make the title bar, the app's header row (title + action buttons), and any table-header strip read as **one continuous band**. Paint all three with `bg.band` (`#F7F5F2`, one shade off `bg.base`) and let the body/content area below them sit on `bg.base` or `bg.surface`. The eye then sees a single piece of chrome at the top of the window instead of three stacked strips with hairline seams — the Claude-Code / Linear look.

Two things make or break it:

- **The chrome color is `bg.band`, and the title bar must use the *same* value.** The title-bar tint, the header row's fill, and the table-header fill are all `bg.band`. If the title bar is `bg.base` and the header row is `bg.band`, you get a visible seam exactly at the title-bar boundary — the bug looks like the title bar "not matching" even though you set it.
- **Watch the outermost container's background.** The most common way the band breaks is an outer layout container painted `bg.base` that bleeds into the title-bar safe area and paints *over* the band color you set on the window. In SwiftUI specifically, the root `VStack`'s `.background(...)` must be `bg.band` (not `bg.base`) so the safe-area region under the unified title bar stays continuous with the header; paint `bg.base`/`bg.surface` only on the body regions below the header. (This is the seam the manga-katana-watcher reforge chased down: the window chrome was correct, but the SwiftUI outer background was `bg.base` and painted over it in the safe area.)

If the worker is a single flat content area with no header row (a one-field utility), you don't need the band — just match the title bar to `bg.base` per the rule above. The band is for workers with a real header (title + buttons) or a table.
...
#### Menu-bar workers: the strip goes white when the window is *reopened*

There's a second white-strip bug, distinct from the focus-loss one above, that only hits **menu-bar workers** (the ones that close to an `NSStatusItem` and keep running). The window looks correct on first launch, then the title-bar strip is white the next time the user reopens it from the menu bar. The cause is the activation-policy toggle: a menu-bar worker flips `NSApplication.setActivationPolicy(.accessory)` when the window closes (so the Dock icon disappears) and back to `.regular` on reopen, and **that policy flip drops the window's transparent-title-bar configuration** (`titlebarAppearsTransparent`, `.fullSizeContentView`, the chrome background). The first-launch setup is gone by the time the window comes back.

The fix is to make the chrome setup idempotent and reapply it every time the window opens, not just once at launch. Pull the window styling into a single `applyWindowChrome(to:)` function and call it: at launch, *and* on every reopen — once before the window shows and again async just after (the policy change lands between frames, so a single pre-show call isn't enough). Reapplying around the `.accessory`⇄`.regular` flip is what keeps the chrome correct across the close/reopen cycle. (This is the exact reforge the manga-katana-watcher app went through: `applyWindowChrome(to:)` called from `attachToMainWindow` at launch and from `openMainWindow` pre-show + post-show.)
...
- **Native — SwiftUI on macOS (default)** — declare the palette as a `Color` extension keyed off the tokens below, then set `.windowStyle(.hiddenTitleBar)`. If the worker has a header row, set the root `VStack`'s `.background(...)` to `Color.bgBand` (not `bgBase`) so the unified title bar reads as one band with the header (see "The header band"); paint `bgBase`/`bgSurface` only on the body regions below. Crucially, also apply the CALayer title-bar fix from the "SwiftUI on macOS" subsection above — `.background` / `.toolbarBackground` alone leave the title-bar strip white when the window loses focus — and if it's a menu-bar worker, reapply that chrome on every reopen (see "Menu-bar workers"). Use `.cornerRadius(12)` on cards, `.cornerRadius(8)` on buttons / inputs, and `Capsule()` for pills. The platform font and HiDPI rendering come for free.
