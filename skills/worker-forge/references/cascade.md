...
- Summarize → HOSTED. Local models lose the thread on long contracts; this is exactly the case for frontier judgment.
- Write output → CODE.

Cascade: `read (CODE) → chunk (CODE) → summarize (HOSTED) → write (CODE)`. The user needs a key for whichever provider they picked in the interview, prompted on first run.
