/* ============================================================
   Triggers editor — a workflow owns a LIST of trigger rules.
   Cron schedules only (multiple allowed); add new types to the
   TRIGGER_TYPES registry. Exposes window.Views.TriggersEditor
   ============================================================ */
(function () {
  "use strict";
  const { e, Icon, Btn, fmtTimestamp } = window.UI;
  ...
  const TYPE_ORDER = ["cron"];

  /* ---------------- cron field validation ----------------
     per-position numeric ranges; each field may be *, a number, a-b range,
     a comma list, or any of those with a /step. weekday allows 0-7. */
  ...
  /* ---------------- bodies per type ---------------- */
  function CronBody({ t, onPatch, ctx }) {
      ...
      e("div", { className: "sched-card", style: { marginTop: 12 } },
        ...
        e("div", null,
          ...
          e("div", { className: "mono", style: { fontSize: 11.5, color: "var(--tx-lo)", marginTop: 3 } },
            (() => { const tz = (ctx && ctx.timezone) || "UTC"; const nx = window.DB.nextCronRun(t.cron, window.DB.NOW_SEC); return nx != null ? "Next run · " + fmtTimestamp(nx, tz) + " · " + window.UI.tzShort(tz, nx * 1000) : "Next run · —"; })()))),
  ...
  /* ---------------- reusable panel (used in TriggersEditor + Workflow edit tab) ---------------- */
  function TriggersPanel({ triggers, setTriggers, ctx }) {
  ...
  Object.assign(window.Views, { TriggersEditor, TriggersPanel });
})();
