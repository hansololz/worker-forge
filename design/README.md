...
- **About card** (last card in the column): a single row of outbound **link buttons** (pill-style: `30px` tall, `--bg-2`, `--line-soft`, `8px` radius, icon + label, `target="_blank"`) — currently just **View on GitHub** (`https://github.com/hansololz/worker-forge`, GitHub brand icon) — followed by a muted footer block (top border, `11.5px` `--tx-dim`) with copyright. The link row carries `paddingTop:14` because the settings `.card-b` only has `4px 18px` padding (it relies on `.set-row`s for vertical rhythm, and this card has none). *(Earlier iterations also had a Version / Check-for-updates row and Release-notes / Report-an-issue links; these were removed.)*
